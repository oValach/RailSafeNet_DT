...
class CustomDataset(VisionDataset):
    ...
    def __getitem__(self, idx):
        ...
        with open(image_path, "rb") as image_file, open(mask_path, "rb") as mask_file:
            ...
            class_labels = np.arange(0,len(cls_remaining)+1)
            number_of_classes = len(class_labels)
            
            # Create binary masks for each class
            binary_masks = torch.zeros((number_of_classes, mask.size(0), mask.size(1)), dtype=torch.uint8)
            for i, class_id in enumerate(class_labels):
                binary_masks[i] = (mask == class_id)

            class_labels = torch.from_numpy(class_labels)
            
            sample = [image, binary_masks.byte(), class_labels.byte()]
            return sample